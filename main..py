...
ydl_opts = {
    ...
    'buffer-size SIZE': 8000,
    'username' : 'zhilong.k.liang@gmail.com',
...
async def play(ctx, *args):
    ...
    try:
      if not voice.is_playing() and not voice.is_paused():
          ...
          song = {"title": TITLE, "url": URL, "duration": DURATION, "thumbnail" : THUMBNAIL}
          del song_queue[0]
          voice.play(discord.PCMVolumeTransformer(FFmpegPCMAudio(song['url'], **FFMPEG_OPTIONS), volume=0.3),
          ...
          embed = discord.Embed(title=song['title'], description='Requested by: \n'+ str(ctx.message.author), color=discord.Color.purple())
        ...
        with YoutubeDL(ydl_opts) as ydl:
          ...
          if info.get('url', None):
              ...
              DURATION = info['entries'][0]['duration']
              THUMBNAIL = info['entries'][0]['thumbnail']
          song = {"title": TITLE, "url": URL, "duration": DURATION, "thumbnail" : THUMBNAIL, "author": str(ctx.message.author)}
          x = divmod(song['duration'], 60)
          y = str(f"{x[0]:02d}") + ":" + str(f"{x[1]:02d}")
          embed = discord.Embed(title="Added to Queue", description="Requested By: \n" + song['author'], color=discord.Color.purple())
          embed.set_thumbnail(url=song['thumbnail'])
...
def play_next(ctx):
    ...
    if song_queue:
        ...
        song = {"title": TITLE, "url": URL, "duration": DURATION, "thumbnail" : THUMBNAIL, "author": str(ctx.message.author)}
        del song_queue[0]
        voice.play(discord.PCMVolumeTransformer(FFmpegPCMAudio(song['url'], **FFMPEG_OPTIONS), volume=0.3), after=lambda e: play_next(ctx))
        voice.is_playing()
        x = divmod(song['duration'], 60)
        y = str(f"{x[0]:02d}") + ":" + str(f"{x[1]:02d}")
...
async def playnext(ctx, *args):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        now = datetime.now() + timedelta(hours=2)
        current_time = now.strftime("%H:%M")
        embed = discord.Embed(title='Playing Next', description='Requested by: \n'+ str(ctx.message.author), color=discord.Color.purple())
        embed.set_thumbnail(url=song['thumbnail'])
        embed.add_field(name='Song',value=song['title'],inline=True)
        embed.add_field(name='Duration', value=y)
        embed.set_footer(text=current_time)
